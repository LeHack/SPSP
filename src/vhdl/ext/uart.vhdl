...
ARCHITECTURE logic OF uart IS
    ...
    constant BAUD_CYCLES  : integer := clk_freq/baud_rate - 1;
    constant OS_CYCLES    : integer := clk_freq/baud_rate/os_rate - 1;
    constant HALF_OS_RATE : integer := os_rate/2;
BEGIN

	--generate clock enable pulses at the baud rate and the oversampling rate
	PROCESS(reset_n, clk)
		VARIABLE count_baud	:	INTEGER RANGE 0 TO BAUD_CYCLES := 0;			--counter to determine baud rate period
		VARIABLE count_os		:	INTEGER RANGE 0 TO OS_CYCLES := 0;	--counter to determine oversampling period
	BEGIN
		...
		ELSIF(clk'EVENT AND clk = '1') THEN
			...
			IF(count_baud < BAUD_CYCLES) THEN			--baud period not reached
			...
			IF(count_os < OS_CYCLES) THEN	--oversampling period not reached
			...
			CASE rx_state IS
				WHEN idle =>																		--idle state
					...
					IF(rx = '0') THEN 																--start bit might be present
						IF(os_count < HALF_OS_RATE) THEN													--oversampling pulse counter is not at start bit center
